...
●`  apt-get install yasm`<h6 id="3">这条命令是针对上面两条命令的，如果你看到nasm/yasm not found的报错提示（你甚至会在apt-get上面折腾好半天，关键词：sudo gedit /etc/apt/sources.list、阿里镜像源、sudo rm /var/lib/apt/lists/lock，跟着报错提示自行百度或者谷歌即可），请使用这条命令安装yasm再执行上面的make命令，没有请忽视这条命令,顺便说一句后面的g++命令也需要安装，方法如同
</h6>
...
## 安装（On ubuntu16.04）| 安装过程中出错异常请参考[第一种方法](#3)
### 推荐先在终端敲上`  sudo su`的命令进入管理员模式
